...
def quran_xml_into_json(
    ...
    return json.dumps(
        quran, default=vars, ensure_ascii=False, indent=(pretty and 4) or None
    )


def translation_xml_into_json(
    file_path, mushaf, source, language, author, pretty=False
):
    with open(file_path, "r") as file:
        content = file.read().encode("utf-8")
    ...
    translation = Translation(
        mushaf, language, source, first_ayah_text, author
    ).surahs_from_xml(root)

    return json.dumps(
        translation, default=vars, ensure_ascii=False, indent=(pretty and 4) or None
    )
...
def main(args):
    ...
    match command:
        ...
        case "translation":
            ...
            json = translation_xml_into_json(
                translation_path, mushaf, source, language, author, pretty
            )
        ...
        case "translation-bulk":
            ...
            for translation in files_in_dir(translations_dir_path):
                ...
                json = translation_xml_into_json(
                    path, mushaf, source, language, author, pretty
                )

                with open(
                    os.path.join(output_dir, f"{language}.{author}.json"),
                    "w",
                    encoding="utf-8",
                ) as file:
                    file.write(json)
